(() => {
  ...
  function applyVehicleView(){
    if (vehicleMode === 'emoji'){
      if (miniEmoji){ miniEmoji.textContent = vehicleEmoji; miniEmoji.style.display='block'; miniEmoji.style.left = (progressPct*100)+'%'; }
      if (miniIcon){  miniIcon.style.display='none'; }
    } else {
      if (miniIcon){  miniIcon.src = vehicleImage; miniIcon.style.display='block'; miniIcon.style.left = (progressPct*100)+'%'; }
  ...
  function setProgressByTimer(){
    ...
    // Update UI: vehicle left% and progress bar width
  ...
  function startTick(){
    ...
    tickTimer = setInterval(() => {
      ...
      if (countdownSec > 0) {
        ...
        tickTimer = null;
        setProgressByTimer(); // ensure it snaps to 100%
        statusEl.textContent = 'Arrived';
        renderETA();
      }
    }, 1000);
  ...
  window.addEventListener('message', (event) => {
    ...
    if (msg.type === 'rv:update'){
      if (typeof msg.rv === 'string') { vehicleImage = msg.rv; vehicleMode = 'image'; }
  ...
  updateLabels();
  renderETA();
  setProgressByTimer(); // ensure initial positions
  applyVehicleView();
})();